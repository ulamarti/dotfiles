...
" Toggle line numbers
nn <silent> <F1> :set nu!<CR> :set rnu!<CR>

" Status
se pt=<F2>

" Toggle spell checking
nn <silent> <F3> :set spell!<CR>
...
se ai             " Auto indent
se aw             " Auto write files
se ar             " Auto read on change
...
se noswf          " Disable swap files
se sb             " Split window below
se undofile       " Enable undo after exit
se cin            " C/C++ indentation
" se cc=80          " Gutter at 80 chars
se sw=3           " Shift width
se sts=3          " Soft tab stop
se ts=3           " Tab stop
se ls=0           " Hide status line
se ttm=0          " Speed up ESC key response
se ut=0           " Faster updates
se pa=**          " Recursive find
se wig=*.o,*.d    " Wild ignore
se tws=10x0       " Terminal window size
se fcs=vert:\▏    " Custom split line
se shm+=Fc        " Avoid most Hit Enter messages
se enc=utf-8      " UTF-8 encoding
se tag=tags;/     " Ctags lookup
se cb=unnamed     " Yank to clipboard
se udir=$HOME/.vim/undo
se ruf=%30(%=%#LineNr#%.50F\ [%{strlen(&ft)?&ft:'none'}]\ %l:%c\ %p%%%)
